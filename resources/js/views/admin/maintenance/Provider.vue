...
import { useProviderStore } from '@/stores/admin/maintenance/useProviderStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useProviderStore();
const { error, processing, token, option } = storeToRefs(useProviderStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Game Provider');
const func = ref(null);
