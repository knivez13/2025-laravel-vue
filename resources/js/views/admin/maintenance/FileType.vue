...
import { useFileTypeStore } from '@/stores/admin/maintenance/useFileTypeStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useFileTypeStore();
const { error, processing, token, option } = storeToRefs(useFileTypeStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('File Type');
const func = ref(null);
