...
import { useVideoTypeStore } from '@/stores/admin/maintenance/useVideoTypeStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useVideoTypeStore();
const { error, processing, token, option } = storeToRefs(useVideoTypeStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Video Type');
const func = ref(null);
