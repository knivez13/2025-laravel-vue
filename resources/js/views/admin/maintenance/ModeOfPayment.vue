...
import { useModePaymentStore } from '@/stores/admin/maintenance/useModePaymentStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useModePaymentStore();
const { error, processing, token, option } = storeToRefs(useModePaymentStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Mode of Payment');
const func = ref(null);
