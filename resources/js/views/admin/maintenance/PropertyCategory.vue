...
import { usePropertyCategoryStore } from '@/stores/admin/maintenance/usePropertyCategoryStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = usePropertyCategoryStore();
const { error, processing, token, option } = storeToRefs(usePropertyCategoryStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Property Category');
const func = ref(null);
