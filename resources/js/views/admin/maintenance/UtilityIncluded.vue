...
import { useUtilityIncludedStore } from '@/stores/admin/maintenance/useUtilityIncludedStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useUtilityIncludedStore();
const { error, processing, token, option } = storeToRefs(useUtilityIncludedStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Utility Included');
const func = ref(null);
