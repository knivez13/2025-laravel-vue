...
import { useGameTypeStore } from '@/stores/admin/maintenance/useGameTypeStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = useGameTypeStore();
const { error, processing, token, option } = storeToRefs(useGameTypeStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Game Type');
const func = ref(null);
