...
import { usePropertyTypeStore } from '@/stores/admin/maintenance/usePropertyTypeStore.js';
const { fnFetch, fnStore, fnUpdate, fnDelete, set_keywords, set_processing, set_rows, set_page, set_sort, tigger_modal } = usePropertyTypeStore();
const { error, processing, token, option } = storeToRefs(usePropertyTypeStore());
const keyword = ref(null);
onBeforeMount(async () => {
    re_fetch();
    await fnFetch();
});
const title = ref('Property Type');
const func = ref(null);
